...
int solution(int m, int n, vector<vector<int>> city_map) {
    ...
    for(int x = 0; x < m; x++){
        for(int y = 0; y<n; y++){
            ...
            dp[x][y][0] += x > 0 ? dp[x-1][y][0] % 20170805 : 0;
            //좌측에 1이 있지 않은 경우엔 방향전환하여 오는 경우도 더한다. (= dp[x-1][y][1])
            dp[x][y][0] += (x > 0 && city_map[x-1][y] != 2) ? dp[x-1][y][1] % 20170805 : 0;
            //위쪽에서 방향전환 없이 오는 경우 (=dp[x][y-1][1])
            dp[x][y][1] += y > 0 ? dp[x][y-1][1] % 20170805 : 0;
            //위쪽에 1이 있지 않은 경우엔 방향전환하여 오는 경우도 더한다. (= dp[x][y-1][0])
            dp[x][y][1] += (y > 0 && city_map[x][y-1] != 2) ? dp[x][y-1][0] % 20170805 : 0;
        }
    }
    answer = (dp[m-1][n-1][0]+ dp[m-1][n-1][1]) % 20170805;
    return answer;
}
